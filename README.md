...
5. Open the TabTune popup and choose **Open Chrome shortcut settings**, or open `chrome://extensions/shortcuts` directly. Each track, volume, and seek control has its own command; three have suggested global shortcuts and the other three are available to bind manually.
...
The default suggested global shortcuts are `Ctrl+Shift+8` for Previous track, `Ctrl+Shift+9` for Next track, and `Ctrl+Shift+0` for Increase volume. macOS uses the corresponding Command shortcuts. All six commands can be rebound in Chrome's shortcut settings.

TabTune detects HTML audio/video elements on any authorized HTTP/HTTPS page, including players that create detached `new Audio()` elements. Volume and seek use the media element directly. Previous/next uses Media Session handlers or matching player controls (including common ARIA labels) when the page exposes them; it does not depend on the site's domain.
