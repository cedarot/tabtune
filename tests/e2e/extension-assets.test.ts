...
describe('built extension contract', () => {
  it('contains a MV3 manifest with all global control commands', () => {
    ...
    const manifest = JSON.parse(readFileSync('dist/manifest.json', 'utf8')) as { manifest_version: number; background: { service_worker: string }; action: { default_popup: string; default_icon?: Record<string, string> }; icons?: Record<string, string>; options_ui: { page: string }; content_scripts?: Array<{ js?: string[]; world?: string; run_at?: string }>; commands: Record<string, { global?: boolean }> };
